...
export async function getDeveloper(username) {
    ...
    const [userRes, repoRes, eventRes] = await Promise.all([
        fetch(`https://api.github.com/users/${clean}`, options),
        fetch(`https://api.github.com/users/${clean}/repos?sort=updated&per_page=100`, options),
        fetch(`https://api.github.com/users/${clean}/events/public?per_page=30`, options),
    ]);
    if (userRes.status === 404)
        return null;
    if (!userRes.ok)
        throw new Error(userRes.status === 403 ? 'GitHub rate limit reached. Try again shortly.' : 'GitHub profile could not be loaded.');
