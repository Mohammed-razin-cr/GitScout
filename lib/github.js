...
export async function getDeveloper(username) {
    ...
    const [userRes, repoRes, eventRes] = await Promise.all([
        fetchGitHub(`https://api.github.com/users/${clean}`),
        fetchGitHub(`https://api.github.com/users/${clean}/repos?sort=updated&per_page=100`),
        fetchGitHub(`https://api.github.com/users/${clean}/events/public?per_page=30`),
    ]);
