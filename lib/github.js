    ...
    for (const token of attempts) {
        try {
            ...
            if (response.ok || response.status === 404)
                return response;
            // An expired or revoked Worker secret returns 401. Keep trying the
            // remaining tokens, then fall back to GitHub's public API instead
            // of incorrectly treating every public username as unavailable.
...
export async function getDeveloper(username) {
    ...
    // Keep the curated profiles available even when the GitHub API is rate-limited
    // or temporarily unavailable from the Worker runtime.
    if (fallback)
        return fallback;
    ...
    if (!userRes?.ok)
        console.warn('[GitScout] GitHub profile lookup did not succeed', {
            username: clean,
            userStatus: userRes?.status ?? 'network-error',
            repositoryStatus: repoRes?.status ?? 'network-error',
            eventStatus: eventRes?.status ?? 'network-error',
        });
