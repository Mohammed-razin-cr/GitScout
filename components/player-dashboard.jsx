...
export function PlayerDashboard({ developer }) {
    ...
    <section className="relative border-b border-primary/10">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap"><Button variant="outline" onClick={copy}><Copy data-icon="inline-start"/>Copy</Button><Button variant="outline" onClick={share}><Share2 data-icon="inline-start"/>Share</Button><Button className="premium-button col-span-2 sm:col-auto" onClick={download}><Download data-icon="inline-start"/>Export card</Button></div>
        </div>
      </div>
    </section>
    ...
    <section id="overview" className="mx-auto grid max-w-7xl scroll-mt-36 gap-10 px-4 py-10 sm:px-6 sm:py-14 lg:grid-cols-[minmax(340px,460px)_1fr] lg:gap-14 lg:px-8">
      ...
      <div id="analytics" className="min-w-0 scroll-mt-36">
        <Reveal><div className="grid grid-cols-2 gap-2 sm:gap-4">{metrics.map(({ icon: Icon, value, label }) => <Card key={label} className="glass-panel group min-w-0 overflow-hidden"><CardContent className="relative p-3 sm:p-6"><Icon className="mb-4 size-5 text-primary transition-transform group-hover:-translate-y-1 sm:mb-6"/><p className="truncate font-heading text-2xl font-black tracking-tight sm:text-4xl"><AnimatedCounter value={value} formatter={compactNumber}/></p><p className="mt-1 text-[9px] uppercase tracking-[.12em] text-muted-foreground sm:text-xs sm:tracking-[.18em]">{label}</p></CardContent></Card>)}</div></Reveal>
